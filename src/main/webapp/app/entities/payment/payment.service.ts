...
export class PaymentService {
  ...
  protected convertDateFromClient(payment: IPayment): IPayment {
    const copy: IPayment = Object.assign({}, payment, {
      dateTime: payment.dateTime && payment.dateTime.isValid() ? payment.dateTime.toJSON() : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((payment: IPayment) => {
        payment.dateTime = payment.dateTime ? moment(payment.dateTime) : undefined;
      });
    }
    return res;
  }
